...
class RemoveCartSuccess extends RemoveCartState {
  RemoveCartSuccess({required this.error, required this.cartDetails, required this.successMessage});
...
class RemoveCartCubit extends Cubit<RemoveCartState> {
  ...
  Future<void> removeCart({required final String providerId}) async {
    try {
      ...
      await cartRepository.removeCart(useAuthToken: true, providerId: providerId).then((final value) {
        emit(
          RemoveCartSuccess(
            error: value["error"],
