...
class VerifyOtpSuccess extends VerifyOtpState {
  VerifyOtpSuccess({this.signinCredential, this.message, required this.authenticationType});
  ...
  Future<void> verifyOtp({
    ...
    try {
      ...
      if (authenticationType == "sms_gateway") {
        final Map<String, dynamic> response = await authRepo.verifyOTPUsingSMSGateway(
          otp: otp,
        ...
        final UserCredential userCredential = await authRepo.verifyOtpUsingFirebase(
          code: otp,
        );
        emit(VerifyOtpSuccess(
      ...
      emit(VerifyOtpFail(findFirebaseError(error.message ?? "")));
    } catch (e) {
