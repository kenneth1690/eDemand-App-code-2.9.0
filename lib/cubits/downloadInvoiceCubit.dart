...
class DownloadInvoiceInProgress extends DownloadInvoiceState {
  ...
  DownloadInvoiceInProgress({required this.bookingId, required this.buttonScreenName});
}

class DownloadInvoiceSuccess extends DownloadInvoiceState {
  DownloadInvoiceSuccess(
      {required this.bookingId, required this.invoiceData, required this.buttonScreenName});
...
class DownloadInvoiceFailure extends DownloadInvoiceState {
  DownloadInvoiceFailure(
      {required this.bookingId, required this.errorMessage, required this.buttonScreenName});
...
class DownloadInvoiceCubit extends Cubit<DownloadInvoiceState> {
  ...
  void downloadInvoice({required final String bookingId, required final String buttonScreenName}) {
    emit(DownloadInvoiceInProgress(bookingId: bookingId, buttonScreenName: buttonScreenName));

    bookingRepository.downloadInvoice(bookingId: bookingId).then((final value) {
      emit(DownloadInvoiceSuccess(
          invoiceData: value, bookingId: bookingId, buttonScreenName: buttonScreenName));
    }).catchError((final e, final st) {
      emit(DownloadInvoiceFailure(
          errorMessage: e.toString(), bookingId: bookingId, buttonScreenName: buttonScreenName));
    });
  }
}
