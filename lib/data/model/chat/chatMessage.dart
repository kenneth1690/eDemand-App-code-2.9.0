...
class ChatMessage {
  ...
  factory ChatMessage.fromJsonAPI(Map<String, dynamic> json) {
    ...
    if (((json["file"] ?? []) as List).isNotEmpty) {
      ...
      documents.addAll((json["file"] as List).map((e) => MessageDocument.fromJson(e)).toList());
  ...
  factory ChatMessage.fromNotificationData(Map json) {
    ...
    final List? file = jsonDecode(json["file"] ?? "");

    if ((file?[0] as List).isNotEmpty) {
      documents.addAll((file![0] as List).map((e) => MessageDocument.fromJson(e)).toList());
    ...
    final List? senderDetails = jsonDecode(json["sender_details"]?.toString() ?? "");
    ...
    return ChatMessage(
      ...
      messageType: messageType,
      sendOrReceiveDateTime:
          DateTime.tryParse(json['last_message_date'].toString())?.toLocal() ?? DateTime.now(),
      senderId: json['sender_id']?.toString() ?? "0",
      isLocallyStored: false,
      receiverId: json['receiver_id'],
  ...
  ChatMessage copyWith({
    ...
    return ChatMessage(
      id: id ?? this.id,
      sendOrReceiveDateTime: sendOrReceiveDateTime ?? this.sendOrReceiveDateTime,
  ...
  factory ChatMessage.fromMap(Map<String, dynamic> map) {
    ...
    return ChatMessage(
        id: map['id'] as String,
        sendOrReceiveDateTime:
            DateTime.fromMillisecondsSinceEpoch(map['sendOrReceiveDateTime'] as int),
        senderId: map['senderId'].toString(),
        isLocallyStored: map['isLocallyStored'] as bool,
        message: map['message'] as String,
        receiverId: map['receiverId'],
        senderDetails: SenderDetails.fromJson(map['senderDetails']),
        files: (map['files'] as List?)?.map((e) => MessageDocument.fromJson(e)).toList() ?? [],
