...
class ChatNotificationsUtils {
  ...
  static void addChatStreamAndShowNotification({required RemoteMessage message}) {
    final chatNotification = ChatNotificationData.fromRemoteMessage(remoteMessage: message);
    notificationStreamController.add(chatNotification);
    if (currentChattingUserHashCode != chatNotification.fromUser.hashCode && Platform.isAndroid) {
  ...
  static Future<void> createChatNotification(
      {required ChatNotificationData chatData, required RemoteMessage message}) async {
    String title = "";
    String body = "";
    if (message.notification != null) {
      ...
      LocalAwesomeNotification.notification.createNotification(
        content: NotificationContent(
            ...
            title: chatData.fromUser.bookingId =="0" ? "New Message": "Booking Id: ${chatData.fromUser.bookingId}",
            body: body,
            locked: false,
            wakeUpScreen: true,
