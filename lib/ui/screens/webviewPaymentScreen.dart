...
class WebviewPaymentScreen extends StatefulWidget {
  const WebviewPaymentScreen({required this.paymentUrl, final Key? key}) : super(key: key);
  final String paymentUrl;

  static Route route(final RouteSettings settings) {
    final arguments = settings.arguments as Map;
    return CupertinoPageRoute(
      builder: (final context) => WebviewPaymentScreen(paymentUrl: arguments['paymentURL']),
    );
  }
      ...
      NavigationDelegate(
        ...
        onNavigationRequest: (final request) {
          if (request.url.startsWith("${baseUrl}app_payment_status") ||
            ...
            if (url.contains('payment_status=Completed') || url.contains('status=successful')) {
              Navigator.pop(context, {'paymentStatus': 'Completed'});
            } else if (url.contains('payment_status=Failed') || url.contains('status=cancelled')) {
        ...
        body: PopScope(
          ...
          child: Scaffold(
            appBar: AppBar(
              ...
              title: CustomText("payment".translate(context: context), color: context.colorScheme.blackColor),
              centerTitle: true,
              elevation: 1,
