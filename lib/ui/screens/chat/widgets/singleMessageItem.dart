...
class _SingleChatMessageItemState extends State<SingleChatMessageItem> {
  ...
  Widget _buildTextMessageWidget({
    ...
    return Row(
      mainAxisAlignment:
          textMessage.senderId == widget.senderId ? MainAxisAlignment.end : MainAxisAlignment.start,
      ...
      children: [
        Flexible(
          child: Container(
            ...
            child: Column(
              crossAxisAlignment: /* (textMessage.receiverId == widget.providerId)
              ...
              children: [
                ...
                SelectableText.rich(
                  TextSpan(
                    ...
                    children: replaceLink(text: textMessage.message).map((data) {
  ...
  Widget _buildImageMessageWidget({
    ...
    Widget singleImageItem({required ChatMessage imageMessage, required int index}) {
        ...
        child: Container(
          ...
          child: imageMessage.isLocallyStored
              ...
              : CustomCachedNetworkImage(
                  networkImageUrl: imageMessage.files[index].fileUrl, fit: BoxFit.contain),
        ),
      );
    }
              ...
              children: [
                Expanded(
                  child: Row(
                    ...
                    children: [
                      Expanded(
                        child: singleImageItem(imageMessage: imageMessage, index: 0),
                      ),
                      if (imageMessage.files.length > 1 && imageMessage.files.length != 3) ...[
                        const SizedBox(
                          width: 5,
                        ),
                        Expanded(
                          child: singleImageItem(imageMessage: imageMessage, index: 1),
                        ),
                      ]
                    ],
                  ),
                        ...
                        if (imageMessage.files.length > 4) ...[
                          ...
                          Expanded(
                            child: Stack(
                              fit: StackFit.expand,
                              children: [
                                singleImageItem(imageMessage: imageMessage, index: 4),
                                    ...
                                    onTap: () {
                                      Navigator.of(context).pushNamed(
                                        ...
                                        arguments: {
                                          ...
                                          "userName": imageMessage.senderId == widget.senderId
                                              ? "you".translate(context: context)
                                              : widget.chatMessage.senderDetails.name,
                                          "images":
                                              imageMessage.files.map((e) => e.fileUrl).toList(),
                                          "messageDateTime": imageMessage.sendOrReceiveDateTime,
                                        },
                                      );
                                    },
                ...
                if (imageMessage.message.trim().isNotEmpty) ...[
                  const SizedBox(
                    height: 5,
                  ),
                  CustomText(
      ...
      child: Column(
        ...
        children: List<Widget>.generate(
              ...
              (index) {
                ...
                return StatefulBuilder(builder: (context, reBuilder) {
                  return GestureDetector(
                    onTap: () async {
                      ...
                      try {
                        if (fileMessage.isLocallyStored) {
                          OpenFilex.open(file.fileUrl);
                        } else {
                          await UiUtils.downloadOrShareFile(
                              url: file.fileUrl, isDownload: true, customFileName: file.fileName);
                        }
                      } catch (_) {
                        UiUtils.showMessage(context, "somethingWentWrong", MessageType.error);
                      }
                      reBuilder(() {
                    ...
                    child: Container(
                      ...
                      child: Column(
                        mainAxisSize: MainAxisSize.min,
                        crossAxisAlignment: (fileMessage.senderId == widget.senderId)
                            ? CrossAxisAlignment.end
                            : CrossAxisAlignment.start,
                        children: [
                          Row(
                              ...
                              if (!fileMessage.isLocallyStored)
                                Container(
                                  ...
                                  decoration: const BoxDecoration(
                                      shape: BoxShape.circle, color: AppColors.whiteColors),
                                  padding: EdgeInsets.all(isDownloading[index] ? 15 : 12),
                          ...
                          if (fileMessage.message.trim().isNotEmpty && index == fileMessage.files.length-1) ...[
                            const SizedBox(
                              height: 5,
                            ),
                            CustomText(
  ...
  Widget _topNameTimeWidget() {
    return Padding(
      padding: EdgeInsets.only(
          bottom: 2,
          right: widget.chatMessage.messageType == ChatMessageType.textMessage ? 0 : 5,
          left: widget.chatMessage.messageType == ChatMessageType.textMessage ? 0 : 5),
      child: Text(
            ...
            builder: (context, constraints) {
              return Align(
                  ...
                  child: Row(
                    ...
                    children: [
                      widget.chatMessage.messageType == ChatMessageType.textMessage
                          ? _buildTextMessageWidget(
                              context: context,
                              constraints: constraints,
                              textMessage: widget.chatMessage,
                            )
                          : widget.chatMessage.messageType == ChatMessageType.imageMessage
                              ? _buildImageMessageWidget(
                                  context: context,
                                  constraints: constraints,
                                  imageMessage: widget.chatMessage,
                                )
                              : widget.chatMessage.messageType == ChatMessageType.fileMessage
                ...
                : widget.isError
                    ? GestureDetector(
                        ...
                        child: Row(
                          mainAxisSize: MainAxisSize.min,
                          children: [
                            const Icon(Icons.refresh, size: 10, color: AppColors.redColor),
                            const SizedBox(
                              width: 2,
                            ),
                            Flexible(
                              child: CustomText(
                                "errorSendingMessageTryAgain".translate(context: context),
                                maxLines: 1,
                                fontSize: 10,
